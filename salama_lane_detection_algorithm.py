...
class lane_data:
    ...
    def extract_ROIs(self, ROI_width: int = 0):

        def __ROI_extractor(lane_pts, ROI_width):

            if ROI_width == 0: ROI_width = self.lane_width // 8
        ...
        return __ROI_extractor(self.left_lane_polyfit_pts, ROI_width),\
            __ROI_extractor(self.right_lane_polyfit_pts, ROI_width)
